...
// PlaylistManifest represents a playlist metadata manifest
...
// PlaylistManifestEntry represents a single entry in a playlist manifest
...
// writePlaylistManifest writes a playlist manifest to disk
func writePlaylistManifest(path string, manifest PlaylistManifest) error {
	...
	file, err := os.Create(path)
	if err != nil {
		return wrapCategory(CategoryFilesystem, fmt.Errorf("creating manifest: %w", err))
	}
	defer file.Close()
